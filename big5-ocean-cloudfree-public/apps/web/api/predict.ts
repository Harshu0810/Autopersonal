...
export default async function handler(req) {
  try {
    // allow preflight (if any)
    if (req.method === "OPTIONS") return new Response(null, { status: 204 });

    // ✅ bail out early on GET
    ...
    let vec = Array.isArray(out) && Array.isArray(out[0]) ? out[0] : (Array.isArray(out) ? out : null);
    if (!vec || vec.length < 5) return json({ error: "Unexpected HF output", raw: out }, 500);
