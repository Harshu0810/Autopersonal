...
export default async function handler(req: Request) {
  try {
    ...
    if (type === "text") {
      ...
      inputText = `Personality assessment: Openness ${scores.O.toFixed(
        3
      )}, Conscientiousness ${scores.C.toFixed(
        3
      )}, Extraversion ${scores.E.toFixed(
        3
      )}, Agreeableness ${scores.A.toFixed(
        3
      )}, Neuroticism ${scores.N.toFixed(3)}. Based on standardized Big Five inventory.`;
    } else {
    ...
    // Fallback to alternative models for better accuracy
    ...
    if (!vec || vec.length < 5) {
      return json({ error: "Unexpected HF output", raw: output }, 500);
    }
